  ...
  const handleSubmit = async (e) => {
    ...
    if (isSignup) {
      try {
        ...
        // 💥 If backend unreachable → MOCK DATA
        if (!res.ok || !data?.data?.user) {
          console.warn("Backend offline — using mock user");

          sessionStorage.setItem("pendingUserId", "mock-user-123");
        ...
        // ✅ REAL API FLOW
        sessionStorage.setItem("pendingUserId", data.data.user.id);
        sessionStorage.setItem("pendingEmail", data.data.user.email);

        navigate("verify-otp");
        return;
      } catch (err) {
        console.warn("Server offline — mock mode enabled");
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="p-8 shadow-xl dark:shadow-2xl dark:shadow-cyan-500/10 dark:border-slate-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6">
            ...
            <Button
              type="submit"
              className="w-full shadow-lg shadow-blue-600/20 dark:shadow-cyan-500/30"
              size="lg"
            >
